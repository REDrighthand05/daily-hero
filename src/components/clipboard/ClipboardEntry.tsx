 ...
 export default function ClipboardEntryComponent({ entry, onDelete, onStar, onClick }: Props) {
   ...
   return (
     <div className="px-3 py-2 border-b border-divider cursor-pointer hover:bg-default-100 transition-colors duration-100" onClick={() => onClick(entry)}>
       <div className="flex justify-between items-start gap-3">
         ...
         <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 shrink-0 transition-opacity duration-100">
           <Button isIconOnly size="sm" variant="light" className="w-6 h-6 min-w-0 text-foreground-400" onClick={(e) => { e.stopPropagation(); handleCopy(); }} title="Copy">
             <Clipboard size={12} />
           </Button>
           <Button isIconOnly size="sm" variant="light" className={`w-6 h-6 min-w-0 ${entry.starred ? "text-warning" : "text-foreground-400"}`} onClick={(e) => { e.stopPropagation(); onStar(entry.id, !entry.starred); }} title={entry.starred ? "Unstar" : "Star"}>
             <Star size={12} />
           </Button>
           <Button isIconOnly size="sm" variant="light" className="w-6 h-6 min-w-0 text-foreground-400" onClick={(e) => { e.stopPropagation(); onDelete(entry.id); }} title="Delete">
             <Trash2 size={12} />
           </Button>
         </div>
       </div>
